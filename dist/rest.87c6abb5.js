...
})({"8lUmV":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "2201d0e687c6abb5";
"use strict";
...
},{}],"lorA5":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "setup", ()=>setup);
parcelHelpers.export(exports, "restAdapter", ()=>restAdapter);
'use strict';
const buildUrl = (base, from, params)=>{
    ...
    if (params) for(const key in params){
        ...
        if (val !== undefined && val !== null) url.searchParams.set(key, typeof val === 'object' ? JSON.stringify(val) : val);
    }
    return url.toString();
};
const request = async (url, options)=>{
    ...
    return {
        data,
        error: res.ok ? null : data?.message || data?.error || res.statusText,
        status: res.status
    };
};
const setup = async ({ url, headers, auth, fetchOptions })=>{
    if (!url) throw new Error('@symbo.ls/fetch rest: url is required');
    return restAdapter(url, headers, auth, fetchOptions);
};
const restAdapter = (baseUrl, defaultHeaders = {}, authConfig, defaultFetchOptions = {})=>{
    ...
    const adapter = {
        ...
        getSession: async ()=>{
            if (!authConfig?.sessionUrl) return token ? {
                token
            } : null;
            const result = await request(resolveUrl(authConfig.sessionUrl, authConfig.baseUrl), getFetchOptions({
                headers: getHeaders()
            }));
        ...
        signOut: async ()=>{
            if (authConfig?.signOutUrl) await request(resolveUrl(authConfig.signOutUrl, authConfig.baseUrl), getFetchOptions({
                method: 'POST',
                headers: getHeaders()
            }));
            token = null;
        ...
        select: ({ from, params, select, limit, offset, order, single, headers, baseUrl: fromBase })=>{
            ...
            if (order) {
                if (typeof order === 'string') allParams.order = order;
                else if (Array.isArray(order)) allParams.order = order.map((o)=>`${o.by}:${o.asc === false ? 'desc' : 'asc'}`).join(',');
                else if (order.by) allParams.order = `${order.by}:${order.asc === false ? 'desc' : 'asc'}`;
            }
            return request(buildUrl(fromBase || baseUrl, from || '', allParams), getFetchOptions({
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}]},["8lUmV"], null, "parcelRequirefc1b", {})

//# sourceMappingURL=rest.87c6abb5.js.map
